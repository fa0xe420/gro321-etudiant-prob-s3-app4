...
from ..models import BonTravail, BonDiagnostic, BonMiseAJour, BonReparation
...
def creer_bon_travail(numero_serie, type_bon, db_path="data/gro321.db"):
    """
    Crée un nouveau bon de travail dans la base de données.
...
def lire_bon(bon_id, db_path="data/gro321.db"):
    ...
    with get_connection(db_path) as conn:
        ...
        cursor.execute(
            """
            SELECT * FROM bons_travail
            WHERE bon_id = ?
        """,
            (bon_id,),
        )
        row_travail = cursor.fetchone()

        if row_travail:
            if row_travail['type_bon'] == 'diagnostic':
                cursor.execute(
                    """
                    SELECT * FROM bons_diagnostic
                    WHERE bon_id = ?
                """,
                    (bon_id,),
                )
                row_diagnostic = cursor.fetchone()
                return BonDiagnostic(
                    row_diagnostic['symptomes'],
                    row_diagnostic['diagnostic'],
                    row_travail['bon_id'],
                    row_travail['numero_serie'],
                    row_travail['date_creation'],
                    row_travail['statut'],
                )

            elif row_travail['type_bon'] == 'mise_a_jour':
                cursor.execute(
                    """
                    SELECT * FROM bons_mise_a_jour
                    WHERE bon_id = ?
                """,
                    (bon_id,),
                )
                row_mis_a_jour = cursor.fetchone()
                return BonMiseAJour(
                    row_mis_a_jour['version_actuelle'],
                    row_mis_a_jour['version_cible'],
                    row_mis_a_jour['mise_a_jour_reussie'],
                    row_travail['bon_id'],
                    row_travail['numero_serie'],
                    row_travail['date_creation'],
                    row_travail['statut'],
            ...
            elif row_travail['type_bon'] == 'reparation':
                cursor.execute(
                    """
                ...
                row_reparation = cursor.fetchone()
                return BonReparation(
                    row_reparation['composant'],
                    row_reparation['probleme'],
                    row_reparation['pieces_utilisees'],
                    row_travail['bon_id'],
                    row_travail['numero_serie'],
                    row_travail['date_creation'],
                    row_travail['statut'],
...
def supprimer_bon(bon_id, db_path="data/gro321.db"):
    ...
    with get_connection(db_path) as conn:
        ...
        cursor.execute("DELETE FROM bons_diagnostic WHERE bon_id = ?", (bon_id,))
        cursor.execute("DELETE FROM bons_mise_a_jour WHERE bon_id = ?", (bon_id,))
        cursor.execute("DELETE FROM bons_reparation WHERE bon_id = ?", (bon_id,))
        cursor.execute("DELETE FROM bons_travail WHERE bon_id = ?", (bon_id,))
